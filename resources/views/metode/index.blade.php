...
@section('content')
    ...
    <div class="container">
        <div class="page-inner">
            <div class="d-flex justify-content-between align-items-center mb-3">
                 {{-- Form Search --}}
                <form action="{{ route('metode.index') }}" method="GET" class="mb-3">
                ...
                <a href="{{ route('metode.create') }}" class="btn btn-primary">Tambah Metode</a>
            </div>

               @if (request('search'))
                <div class="mb-2">
                    <small class="text-muted">
                        Ditemukan {{ $metode->total() }} Metode Pembayaran untuk kata kunci
            ...
            <div class="table-responsive" >
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        @forelse ($metode as $index => $item)
                            <tr>
                                <td>{{ $metode->firstItem() + $index }}.</td>
                                <td>{{ $item->name }}</td>
                                  <td>
                                    @if ($item->img)
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li>
                                                <a href="{{ route('metode.show', $item->id) }}"
    ...
    <script>
        function deleteKategori(id) {
            Swal.fire({
                title: 'Yakin hapus Metode Pembayaran ini?',
